# Parallel Explore Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Run multiple implementation approaches simultaneously in separate git worktrees, evaluate by test pass rate (quality as tiebreaker), apply the winner.

**Architecture:** New `skills/parallel-explore/SKILL.md` + `memory/parallel-explore-log.md`. Self-contained — no changes to existing skills.

**Tech Stack:** Markdown skill file, Python pytest content-checks.

**Spec:** `docs/superpowers/specs/2026-03-21-parallel-explore-design.md`
...
| `tests/test_skills.py` | Modify — add `"parallel-explore"` to ALL_SKILLS |
| `tests/test_memory.py` | Modify — add `"parallel-explore-log.md"` to REQUIRED_FILES |
...
- [ ] **Step 1: Add to ALL_SKILLS and REQUIRED_FILES**

In `tests/test_skills.py`: add `"parallel-explore"` to `ALL_SKILLS`.
In `tests/test_memory.py`: add `"parallel-explore-log.md"` to `REQUIRED_FILES`.

- [ ] **Step 2: Run tests — verify they fail**

```
python -m pytest tests/test_skills.py::test_skill_exists[parallel-explore] tests/test_skills.py::test_skill_frontmatter[parallel-explore] tests/test_memory.py::test_memory_file_exists[parallel-explore-log.md] -v
```
...
git commit -m "test: add parallel-explore to ALL_SKILLS and REQUIRED_FILES (red)"
```

---

## Task 2: Create `memory/parallel-explore-log.md`

- [ ] **Step 1: Create seed file**

Exact content:
```markdown
# Parallel Explore Log
<!-- parallel-explore appends one session block per run -->
<!-- Each block records all approaches, scores, and the winning strategy -->
```

- [ ] **Step 2: Run test**

```
python -m pytest tests/test_memory.py::test_memory_file_exists[parallel-explore-log.md] -v
```
...
```bash
git add memory/parallel-explore-log.md
git commit -m "feat: add memory/parallel-explore-log.md seed file"
```

---

## Task 3: Create `skills/parallel-explore/SKILL.md`
...
```
python -m pytest tests/test_skills.py::test_skill_exists[parallel-explore] tests/test_skills.py::test_skill_frontmatter[parallel-explore] -v
```
...
```bash
git add skills/parallel-explore/SKILL.md
git commit -m "feat: add parallel-explore skill"
```

---

## Task 4: Update manifest + full suite

- [ ] **Step 1: Update skill-pattern-manifest.md**

Add after `### task-queue`:
...
git commit -m "chore: add parallel-explore to skill-pattern-manifest"
```
